...
export class UsuarioService {
  ...
  protected convertDateFromClient(usuario: IUsuario): IUsuario {
    return Object.assign({}, usuario, {
      birthdate: usuario.birthdate?.isValid() ? usuario.birthdate.format(DATE_FORMAT) : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((usuario: IUsuario) => {
        usuario.birthdate = usuario.birthdate ? dayjs(usuario.birthdate) : undefined;
      });
    }
    return res;
  }
